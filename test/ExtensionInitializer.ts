...
import type * as Ltex from "../src/extension";
import TestTools from "./TestTools";

export default class ExtensionInitializer {
	private static _languageClient: CodeLanguageClient.LanguageClient | null =
		null;
	...
	public static async initialize(): Promise<void> {
		...
		Code.workspace
			...
			.update("trace.server", "messages", Code.ConfigurationTarget.Global);
		...
		if (api.clientOutputChannel == null) {
			return Promise.reject(
				new Error("Client output channel not initialized."),
			);
		}
		...
		if (api.serverOutputChannel == null) {
			return Promise.reject(
				new Error("Server output channel not initialized."),
			);
		}
		...
		for (let i: number = 0; i < 120; i++) {
			const diagnostics: Code.Diagnostic[] = Code.languages.getDiagnostics(
				document.uri,
			);
		...
		if (!ltexReady)
			return Promise.reject(new Error("LTeX not successfully initialized."));
	}
		...
		if (ExtensionInitializer._languageClient != null) {
			...
			return ExtensionInitializer.initialize().then(
				() =>
					new Promise(
						(
							resolve: (value: CodeLanguageClient.LanguageClient) => void,
