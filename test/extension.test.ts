...
describe("Test extension (end-to-end)", () => {
	async function assertCheckingResult(
	...
	it("Test arrays in ltex.enabled", async () => {
		try {
			Code.workspace
				...
				.update("enabled", ["markdown"], Code.ConfigurationTarget.Global);

			const markdownDocument: Code.TextDocument = await TestTools.createNewFile(
				"markdown",
				"This is an *test*.",
			);
			await assertCheckingResult(markdownDocument);

			const latexDocument: Code.TextDocument = await TestTools.createNewFile(
				"latex",
				"This is an *test*.",
			);
			await TestTools.sleep(5000);
			...
			Code.workspace
				.getConfiguration("ltex")
				.update("enabled", undefined, Code.ConfigurationTarget.Global);
		}
	});
});
