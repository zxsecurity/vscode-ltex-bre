...
describe("Test commands", () => {
	...
	it("Test ltex.clearDiagnosticsInCurrentDocument and ltex.checkCurrentDocument", async () => {
		...
		await Code.commands.executeCommand(
	...
	it("Test ltex.clearAllDiagnostics and ltex.checkCurrentDocument", async () => {
		const document1: Code.TextDocument = await TestTools.createNewFile(
			"latex",
			"This is an \\textbf{test}.",
		);
		const document2: Code.TextDocument = await TestTools.createNewFile(
			"latex",
			"This is an \\textbf{test}.",
		);
		await TestTools.sleep(10000);
	...
	it("Test ltex.checkAllDocumentsInWorkspace", async () => {
		...
		const workspaceFolderPath: string =
			Code.workspace.workspaceFolders[0].uri.fsPath;
		const fileUri1: Code.Uri = Code.Uri.file(
			Path.join(workspaceFolderPath, "test1.tex"),
		);
		const fileUri2: Code.Uri = Code.Uri.file(
			Path.join(workspaceFolderPath, "test2.md"),
		);
		const fileUri3: Code.Uri = Code.Uri.file(
			Path.join(workspaceFolderPath, "test3.txt"),
		);

		const textEncoder: Util.TextEncoder = new Util.TextEncoder();
		await Code.workspace.fs.writeFile(
			fileUri1,
			textEncoder.encode("This is an test.\n"),
		);
		await Code.workspace.fs.writeFile(
			fileUri2,
			textEncoder.encode("This is an test.\n"),
