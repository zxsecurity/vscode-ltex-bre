...
export class I18n {
	...
	public static initialize(context: Code.ExtensionContext): void {
		I18n._language = I18n.getLanguage();
		Logger.log(`Setting LTeX UI language to '${I18n._language}'.`);
		Logger.log("Loading i18n messages...");
		I18n._messages = I18n.loadMessages(context);
		Logger.log("Loading default i18n messages...");
		I18n._defaultMessages = I18n.loadDefaultMessages(context);
	}
	...
	public static getLanguage(): string {
		...
		if (codeNlsConfigStr != null) {
			try {
				const codeNlsConfig: VSCodeNlsConfig = JSON.parse(
					codeNlsConfigStr,
				) as VSCodeNlsConfig;
	...
	public static loadMessages(context: Code.ExtensionContext): MessageObject {
		let language: string = I18n._language.replace(/[^A-Za-z0-9\-_]/g, "");
	...
	public static loadDefaultMessages(
		context: Code.ExtensionContext,
	): MessageObject {
		return I18n.loadMessagesFile(
			Path.resolve(context.extensionPath, "i18n", "messages.nls.json"),
		);
	}
	...
	public static getMessage(key: string): string {
		if (Object.hasOwn(I18n._messages, key)) {
			return I18n._messages[key];
		} else if (Object.hasOwn(I18n._defaultMessages, key)) {
			return I18n._defaultMessages[key];
		} else {
	...
	public static i18n(key: string, ...messageArguments: any[]): string {
		return I18n.getMessage(key).replace(
			/\{(\d+)\}/g,
			(_match: string, groups: string[]) => {
				return String(messageArguments[parseInt(groups[0], 10)]);
			},
		);
	}
}
