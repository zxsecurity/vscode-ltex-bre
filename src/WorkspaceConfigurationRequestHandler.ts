...
export default class WorkspaceConfigurationRequestHandler {
	...
	private mergeSettings(
		...
		const userSettingValue: LanguageSpecificSettingValue =
			this._externalFileManager.getSettingValue(
				uri,
				settingName,
		...
		WorkspaceConfigurationRequestHandler.mergeLanguageSpecificSettingValue(
			result,
			workspaceFolderSettingValue,
		);

		for (const language in result) {
			if (!Object.hasOwn(result, language)) continue;
			result[language] =
	...
	private static mergeLanguageSpecificSettingValue(
		value1: LanguageSpecificSettingValue,
		value2: LanguageSpecificSettingValue | undefined,
	): void {
		if (value2 == null) return;

		for (const language in value2) {
			if (!Object.hasOwn(value2, language)) continue;
			if (!Object.hasOwn(value1, language)) value1[language] = [];
