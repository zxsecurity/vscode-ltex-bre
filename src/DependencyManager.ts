...
import type * as CodeLanguageClient from "vscode-languageclient/node";
...
export default class DependencyManager {
	...
	private static readonly _offlineInstructionsUrl: string =
	...
	public constructor(context: Code.ExtensionContext) {
		...
		if (vscodeLtexExtension == null)
			throw new Error(i18n("couldNotGetVscodeLtexVersion"));
	...
	private static normalizePath(path: string | null | undefined): string | null {
		...
		return path
			.replace(
				/\$(.+?)($|\/|\\)/,
				(_match: string, name: string, seperator: string) =>
	...
	private static async downloadFile(
		...
		return new Promise(
			(resolve: () => void, reject: (reason?: any) => void) => {
				Https.get(
					DependencyManager.parseUrl(urlStr),
					(response: Http.IncomingMessage) => {
						if (
							...
							if (response.headers.location == null) {
								reject(
									new Error(
										i18n(
											"receivedRedirectionStatusCodeWithoutLocationHeader",
											response.statusCode,
										),
									),
								);
							...
							DependencyManager.downloadFile(
							...
							reject(
								new Error(
									i18n("requestFailedWithStatusCode", response.statusCode),
								),
							);
							return;
						}
						...
						codeProgress.updateTask(
							0,
							totalBytes > 0
								? `${origTaskName}  0MB/${totalMb}MB`
								: origTaskName,
						);
						...
						if (totalBytes > 0) {
							response.on("data", (chunk: any) => {
								...
								if (now - lastTaskNameUpdate >= 500) {
									...
									const downloadedMb: number = Math.round(
										downloadedBytes / 1e6,
									);
	...
	private async installDependency(
		...
		if (url.pathname == null)
			throw new Error(i18n("couldNotGetPathNameFromUrl", urlStr));
		const archiveName: string = Path.basename(url.pathname);
		const archiveType: string =
			Path.extname(archiveName) === ".zip" ? "zip" : "tar.gz";
		const tmpDirPath: string = Fs.mkdtempSync(
			Path.join(this._context.extensionPath, "tmp-"),
		);
		...
		for (let i: number = 0; i < fileNames.length; i++) {
			...
			if (stats.isDirectory()) {
				if (extractedDirPath == null) {
					...
					Logger.warn(
						i18n(
							"foundMultipleDirectoriesAfterExtraction",
							extractedDirPath,
							filePath,
						),
					);
				...
				try {
					...
					Logger.warn(
						i18n("couldNotDeleteLeavingTemporaryFileOnDisk", filePath),
						e,
					);
				}
			}
		...
		const targetDirPath: string = Path.join(
			...
			"lib",
			Path.basename(extractedDirPath),
		);
		...
		if (targetExists) {
			Logger.warn(
				i18n(
					"didNotMoveAsTargetAlreadyExists",
					extractedDirPath,
					targetDirPath,
				),
			);
		...
		try {
			...
			Logger.warn(
				i18n("couldNotDeleteLeavingTemporaryDirectoryOnDisk", tmpDirPath),
				e,
			);
		}
	...
	private async installLtexLs(): Promise<void> {
		...
		return Code.window.withProgress(
			progressOptions,
			async (
				progress: Code.Progress<{ increment?: number; message?: string }>,
			): Promise<void> => {
				const codeProgress: ProgressStack = new ProgressStack(
					i18n("downloadingAndExtractingLtexLs"),
					progress,
				);
				...
				await this.installDependency(
					ltexLsUrl,
					ltexLsHashDigest,
					`ltex-ls-plus ${DependencyManager._toBeDownloadedLtexLsVersion}`,
					codeProgress,
				);
			},
		);
	...
	private searchBundledLtexLs(libDirPath: string): string | null {
		...
		const ltexLsVersion: string | null =
			this.getLatestLtexLsVersion(ltexLsVersions);
		return ltexLsVersion != null
			? Path.join(libDirPath, `ltex-ls-plus-${ltexLsVersion}`)
			: null;
	}
	...
	public async install(): Promise<boolean> {
		...
		try {
			...
			Logger.log("");
			this._ltexLsPath = DependencyManager.normalizePath(
				workspaceConfig.get("ltex-ls.path", ""),
			);
			...
			Logger.log(
				i18n(
					"youMightWantToTryOfflineInstallationSee",
					DependencyManager._offlineInstructionsUrl,
				),
			);
			Logger.showClientOutputChannel();
			return this.showOfflineInstallationInstructions(
				i18n("couldNotInstallLtexLs"),
			);
		}

		Logger.log("");
		Logger.log(i18n("usingLtexLsFrom", this._ltexLsPath));
		this._javaPath = DependencyManager.normalizePath(
			workspaceConfig.get("java.path"),
		);
		...
		if (await this.test()) {
			...
			Logger.showClientOutputChannel();
			return await this.showOfflineInstallationInstructions(
				i18n("couldNotRunLtexLs"),
			);
		}
	}

	private async showOfflineInstallationInstructions(
		message: string,
	): Promise<boolean> {
		return new Promise((resolve: (value: boolean) => void) => {
			Code.window
				.showErrorMessage(
					`${message} ${i18n("youMightWantToTryOfflineInstallation")}`,
					i18n("tryAgain"),
					i18n("offlineInstructions"),
				)
				.then(async (selectedItem: string | undefined) => {
					if (selectedItem === i18n("tryAgain")) {
						...
						Code.env.openExternal(
							Code.Uri.parse(DependencyManager._offlineInstructionsUrl),
						);
					}
	...
	private async test(): Promise<boolean> {
		const executable: CodeLanguageClient.Executable =
			await this.getLtexLsExecutable();
		...
		try {
			childProcess = ChildProcess.spawnSync(
				executable.command,
				executable.args,
				executableOptions,
			);
		...
		if (childProcess.status === 0 && childProcess.stdout.includes("ltex-ls")) {
			try {
				...
				if (Object.hasOwn(versionInfo, "ltex-ls")) {
					ltexLsVersion = versionInfo["ltex-ls"];
				}

				if (Object.hasOwn(versionInfo, "java")) {
					const match: RegExpMatchArray | null =
						versionInfo.java.match(/(\d+)(?:\.(\d+))?/);

					if (match != null && match.length >= 3) {
						javaVersion = versionInfo.java;
						javaMajorVersion = parseInt(match[1], 10);
						if (javaMajorVersion === 1)
							javaMajorVersion = parseInt(match[2], 10);
					}
				}
		...
		if (success) {
			...
			if (childProcess.status != null && childProcess.status !== 0) {
				Logger.log(
					i18n("ltexLsTerminatedWithNonZeroExitCode", childProcess.status),
				);
	...
	public async getLtexLsExecutable(): Promise<CodeLanguageClient.Executable> {
		...
		for (const name in process.env) {
			if (Object.hasOwn(process.env, name) && name !== "JAVA_HOME") {
		...
		const ltexLsScriptPath: string = Path.join(
			this._ltexLsPath!,
			"bin",
			DependencyManager._isWindows ? "ltex-ls-plus.bat" : "ltex-ls-plus",
		);

		const workspaceConfig: Code.WorkspaceConfiguration =
			Code.workspace.getConfiguration("ltex");
		const initialJavaHeapSize: number | undefined = workspaceConfig.get(
			"java.initialHeapSize",
		);
		const maximumJavaHeapSize: number | undefined = workspaceConfig.get(
			"java.maximumHeapSize",
		);
		...
		if (DependencyManager._isWindows) {
			return {
				command: `"${ltexLsScriptPath}"`,
				args: [],
				options: { env: env, shell: true },
			};
		} else {
	...
	public static getDebugServerOptions(): CodeLanguageClient.ServerOptions | null {
		...
		const line: string = output.substring(
