...
export default class ExternalFileManager {
	...
	private _watchers: {
		[settingName: string]: { [filePath: string]: Fs.FSWatcher };
	};
	...
	public updateWatchers(uri: Code.Uri, settingName: string): void {
		...
		const watchers: { [filePath: string]: Fs.FSWatcher } =
			this._watchers[settingName];

		for (const filePath in watchers) {
			if (!Object.hasOwn(watchers, filePath)) continue;
			oldExternalFilePaths.add(filePath);
		}

		const externalFiles: LanguageSpecificExternalFiles =
			this.getExternalFilesFromSetting(uri, settingName);
		...
		for (const language in externalFiles) {
			if (!Object.hasOwn(externalFiles, language)) continue;
		...
		for (const externalFile of newExternalFiles) {
			...
			if (!oldExternalFilePaths.has(filePath)) {
				...
				try {
					...
					watcher = Fs.watch(
						filePath,
						this.onFileWatcherEvent.bind(this, filePath),
					);
	...
	public getFirstExternalFilePath(
		uri: Code.Uri,
		settingName: string,
		scope: Code.ConfigurationTarget,
		language: string,
	): string | null {
		...
		return Object.hasOwn(externalFiles, language) &&
			externalFiles[language].length > 0
			? externalFiles[language][0].resolvedPath
			: null;
	}
	...
	private getExternalFilesFromSetting(
		uri: Code.Uri,
		settingName: string,
		language: string | null = null,
	): LanguageSpecificExternalFiles {
		const resourceConfig: Code.WorkspaceConfiguration =
			Code.workspace.getConfiguration(
				"ltex",
				ExternalFileManager.preprocessUri(uri),
			);
		...
		for (const scope of [
			...
			const settingAnalysis: SettingAnalysis = this.analyzeSetting(
				...
				scope,
				language,
				settingValuePerScope,
			);
			const scopeExternalFiles: LanguageSpecificExternalFiles =
				settingAnalysis.externalFiles;

			for (const curLanguage in scopeExternalFiles) {
				if (!Object.hasOwn(scopeExternalFiles, curLanguage)) continue;
				if (!Object.hasOwn(externalFiles, curLanguage)) {
	...
	private analyzeSetting(
		uri: Code.Uri,
		settingName: string,
		scope: Code.ConfigurationTarget,
		language: string | null = null,
		settingValuePerScope: SettingValuePerScope | undefined = undefined,
	): SettingAnalysis {
		...
		switch (scope) {
			case Code.ConfigurationTarget.Global: {
				...
				settingValue =
					settingValuePerScope != null
						? settingValuePerScope.globalValue
						: null;
				break;
			}
			case Code.ConfigurationTarget.Workspace: {
				...
				settingValue =
					settingValuePerScope != null
						? settingValuePerScope.workspaceValue
						: null;
				break;
			}
			case Code.ConfigurationTarget.WorkspaceFolder: {
				...
				settingValue =
					settingValuePerScope != null
						? settingValuePerScope.workspaceFolderValue
						: null;
				break;
			}
		}
		...
		for (const curLanguage of languages) {
			...
			if (settingValue != null && Object.hasOwn(settingValue, curLanguage)) {
				for (const entry of settingValue[curLanguage]) {
					if (!entry.startsWith(":")) continue;
					const originalFilePath: string = entry.substr(1);
					let filePath: string =
						ExternalFileManager.normalizePath(originalFilePath);
			...
			if (defaultDirPath != null) {
				const curLanguageSuffix: string = curLanguage.replace(
					/[^A-Za-z0-9\-_]/,
					"",
				);
				externalFiles[curLanguage].push({
					...
					resolvedPath: Path.join(
						defaultDirPath,
						`ltex.${settingName}.${curLanguageSuffix}.txt`,
					),
					explicit: false,
				});
			}
	...
	public getWorkspaceSettingsDirPath(): string | null {
		...
		const workspaceDirUri: Code.Uri = ExternalFileManager.postprocessUri(
	...
	public getWorkspaceFolderSettingsDirPath(uri: Code.Uri): string | null {
		...
		const workspaceFolderDirUri: Code.Uri = ExternalFileManager.postprocessUri(
			workspaceFolder.uri,
		);
	...
	private onFileWatcherEvent(filePath: string, eventType: string): void {
		if (!Fs.existsSync(filePath)) {
			Logger.log(
				i18n("stoppedWatchingExternalSettingFileDueToDeletion", filePath),
			);

			for (const settingName of ExternalFileManager._settingNames) {
				if (!Object.hasOwn(this._contents[settingName], filePath)) {
					continue;
				}
			...
			for (const settingName of ExternalFileManager._settingNames) {
				if (!Object.hasOwn(this._contents[settingName], filePath)) continue;
				this._contents[settingName][filePath] = contents;
			}
		}
	}
	...
	public getSettingValue(
		...
		const externalFiles: LanguageSpecificExternalFiles =
			settingAnalysis.externalFiles;
		const settingValue: LanguageSpecificSettingValue =
			settingAnalysis.settingValue != null ? settingAnalysis.settingValue : {};
		...
		for (const language in externalFiles) {
			...
			for (const externalFile of externalFiles[language]) {
				this.loadFileAndPushToArray(
					externalFile.resolvedPath,
					settingName,
					result[language],
				);
			}
		...
		for (const language in settingValue) {
			if (!Object.hasOwn(settingValue, language)) continue;
			if (!Object.hasOwn(result, language)) result[language] = [];

			for (const entry of settingValue[language]) {
				if (
					entry.startsWith(":") &&
					Object.hasOwn(settingAnalysis.externalFiles, language)
				) {
					const filePath: string | null = ExternalFileManager.getResolvedPath(
						settingAnalysis.externalFiles[language],
						entry.substr(1),
					);
	...
	private loadFileAndPushToArray(
		filePath: string,
		settingName: string,
		array: string[],
	): void {
		if (!Object.hasOwn(this._contents[settingName], filePath)) return;
	...
	private static getResolvedPath(
		filePathInfos: ExternalFile[],
	...
	public appendToFile(
		...
		const externalFileKnown: boolean = Object.hasOwn(
			this._contents[settingName],
			filePath,
		);
		let contents: string;
		...
		try {
			...
			Logger.error(
				i18n("couldNotAppendNewEntriesToExternalSettingFile", filePath),
				e,
			);
			return;
		}
	...
	private ensureParentDirExists(filePath: string): void {
		...
		if (!Fs.existsSync(parentDirPath)) {
			try {
				...
				Logger.log(
					i18n(
						"createdDirectoryForExternalSettingFile",
						parentDirPath,
						filePath,
					),
				);
				...
				Logger.warn(
					i18n(
						"couldNotCreateDirectoryForExternalSettingFile",
						parentDirPath,
						filePath,
					),
					e,
