...
export default class StatusPrinter {
	...
	public async print(): Promise<void> {
		...
		if (this._languageClient != null) {
			try {
				ltexLsStatus = await this._languageClient.sendRequest(
		...
		Logger.log(
			i18n(
				"extensionDirPath",
				StatusPrinter.formatString(this._context.extensionPath),
			),
		);
		Logger.log(
			i18n(
				"userSettingsDirPath",
				StatusPrinter.formatString(
					this._externalFileManager.getUserSettingsDirPath(),
				),
			),
		);
		Logger.log(
			i18n(
				"workspaceSettingsDirPath",
				StatusPrinter.formatString(
					this._externalFileManager.getWorkspaceSettingsDirPath(),
				),
			),
		);
		...
		const activeTextEditor: Code.TextEditor | undefined =
			Code.window.activeTextEditor;
		if (activeTextEditor != null)
			activeDocumentUri = activeTextEditor.document.uri;

		let workspaceFolderSettingsDirPath: string | null = null;

		if (activeDocumentUri != null) {
			workspaceFolderSettingsDirPath =
		...
		Logger.log(
			i18n("processId", StatusPrinter.formatNumber(ltexLsStatus.processId)),
		);
		Logger.log(
			i18n(
				"wallClockDuration",
		...
		Logger.log(
			i18n(
				"cpuDuration",
				StatusPrinter.formatDuration(ltexLsStatus.cpuDuration),
			),
		);
		Logger.log(
			i18n("cpuUsage", StatusPrinter.formatFraction(ltexLsStatus.cpuUsage)),
		);
		Logger.log(
			i18n("totalMemory", StatusPrinter.formatMemory(ltexLsStatus.totalMemory)),
		);
		Logger.log(
			i18n("usedMemory", StatusPrinter.formatMemory(ltexLsStatus.usedMemory)),
		);
		Logger.log(
			i18n(
				"isBusyChecking",
		...
		Logger.log(
			i18n(
				"documentUriBeingChecked",
				StatusPrinter.formatString(ltexLsStatus.documentUriBeingChecked),
			),
		);

		const watchedExternalFiles: string[] =
			this._externalFileManager.getAllWatchedExternalFiles();
