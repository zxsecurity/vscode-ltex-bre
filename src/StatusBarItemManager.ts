...
export default class StatusBarItemManager {
	...
	private update(): void {
		...
		for (const token of tokens) {
			if (
				now - this._progressMap[token].startTime >=
				StatusBarItemManager.checkingStatusDelay
			) {
		...
		const visible: boolean | undefined = Code.workspace
			.getConfiguration("ltex")
			.get("statusBarItem");
	...
	public setStatusToReady(): void {
		...
		setTimeout(
			...
			StatusBarItemManager.readyStatusDuration,
		);
	}

	public handleProgressNotification(
		params: ProgressParams<
			| CodeLanguageClient.WorkDoneProgressBegin
			| CodeLanguageClient.WorkDoneProgressReport
			| CodeLanguageClient.WorkDoneProgressEnd
		>,
	): void {
		let token: {
			uri: string;
		...
		try {
			...
			Logger.warn(
				i18n("couldNotParseTokenInProgressNotification", params.token, params),
			);
			return;
		}
		...
		if (token.operation !== "checkDocument") {
			Logger.warn(
				i18n("unknownOperationInProgressNotification", token.operation, params),
			);
			return;
		}
		...
		if (params.value.kind === "begin") {
			...
			setTimeout(
				this.update.bind(this),
				StatusBarItemManager.checkingStatusDelay + 100,
			);
		} else if (params.value.kind === "end") {
			if (Object.hasOwn(this._progressMap, params.token)) {
	...
	public setStatusToDisabled() {
		...
		setTimeout(() => {
			this._statusBarItem.hide();
		}, StatusBarItemManager.disabledStatusDuration);
	}
}
