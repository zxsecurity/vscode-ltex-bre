...
import type ExternalFileManager from "./ExternalFileManager";
import * as Extension from "./extension";
import { i18n } from "./I18n";
import Logger from "./Logger";
import ProgressStack from "./ProgressStack";
import type StatusBarItemManager from "./StatusBarItemManager";
...
export default class CommandHandler {
	...
	public constructor(
		...
		context.subscriptions.push(
			Code.commands.registerCommand("ltex.close", this.close.bind(this)),
		);
		context.subscriptions.push(
			Code.commands.registerCommand(
				"ltex.reportBug",
	...
	private async checkDocument(
		...
		try {
			...
			await Promise.race([
				this._languageClient.needsStart()
					? this._languageClient.start()
					: Promise.resolve(),
				new Promise(
					(_resolve: (value: any) => void, reject: (e: Error) => void) => {
						setTimeout(
							() => reject(new Error(i18n("ltexNotInitialized"))),
							30000,
						);
					},
				),
			]);
			result = await this._languageClient.sendRequest(
		...
		if (result.success) {
			...
			Code.window.showErrorMessage(
				i18n("couldNotCheckDocument", uri.fsPath, result.errorMessage),
			);
			return Promise.resolve(false);
		}
	}
	...
	private async checkSelection(): Promise<boolean> {
		...
		const textEditor: Code.TextEditor | undefined =
			Code.window.activeTextEditor;
	...
	private async checkCurrentDocument(): Promise<boolean> {
		...
		const lastLine: Code.TextLine = textEditor.document.lineAt(
			textEditor.document.lineCount - 1,
		);
		const docText: Code.Selection = new Code.Selection(
		...
		return Code.window.withProgress(
			...
			async (
				...
				const codeProgress: ProgressStack = new ProgressStack(
					i18n("checkingAllDocumentsInWorkspace"),
					progress,
				);
				...
				const fileExtensions: string[] =
					CommandHandler.getEnabledFileExtensions();
				if (fileExtensions.length === 0) return Promise.resolve(true);
				const fileExtensionWildcard: string = fileExtensions.join(",");

				const uris: Code.Uri[] = await CommandHandler.findFiles(
					`**/*.{${fileExtensionWildcard}}`,
					token,
				);
				uris.sort((lhs: Code.Uri, rhs: Code.Uri) =>
					lhs.fsPath.localeCompare(rhs.fsPath),
				);
				...
				if (n > 0) {
					...
					Code.window.showErrorMessage(
						Code.workspace.workspaceFolders == null
	...
	private static async findFiles(
		...
		return await Code.workspace.findFiles(
			globPattern,
			undefined,
			undefined,
			_token,
	...
	private async clearDiagnosticsInCurrentDocument(): Promise<boolean> {
		...
		const textEditor: Code.TextEditor | undefined =
			Code.window.activeTextEditor;
		if (textEditor != null)
			diagnosticCollection.set(textEditor.document.uri, undefined);
	...
	private async openMarkdownExample(): Promise<void> {
		await Code.workspace.openTextDocument({
			language: "markdown",
...
`,
	...
	private addToDictionary(params: any): void {
		this.addToLanguageSpecificSetting(
	...
	private addToLanguageSpecificSetting(
		uri: Code.Uri,
		settingName: string,
		entries: LanguageSpecificSettingValue,
	): void {
		...
		if (scopeString == null || scopeString.endsWith("ExternalFile")) {
			this.addToLanguageSpecificSettingExternalFile(
				uri,
				settingName,
				scopes,
				entries,
			...
			this.addToLanguageSpecificSettingInternalSetting(
				uri,
				settingName,
				scopes,
				entries,
	...
	private addToLanguageSpecificSettingExternalFile(
		uri: Code.Uri,
		settingName: string,
		scopes: Code.ConfigurationTarget[],
		entries: LanguageSpecificSettingValue,
	): void {
		for (const language in entries) {
			if (!Object.hasOwn(entries, language)) continue;
			let settingWritten: boolean = false;

			for (const scope of scopes) {
				const externalFilePath: string | null =
					this._externalFileManager.getFirstExternalFilePath(
						uri,
						settingName,
						scope,
						language,
				...
				if (externalFilePath != null) {
					this._externalFileManager.appendToFile(
						externalFilePath,
						settingName,
						entries[language],
					);
	...
	private async addToLanguageSpecificSettingInternalSetting(
		uri: Code.Uri,
		settingName: string,
		scopes: Code.ConfigurationTarget[],
		entries: LanguageSpecificSettingValue,
		...
		const settingValue: { [language: string]: string[] } = resourceConfig.get(
			settingName,
			{},
		);
		...
		for (const language in entries) {
			if (!Object.hasOwn(entries, language)) continue;
			let languageSettingValue: string[] =
				settingValue[language] != null ? settingValue[language] : [];
			languageSettingValue = languageSettingValue.concat(entries[language]);
			settingValue[language] =
				WorkspaceConfigurationRequestHandler.cleanUpWorkspaceSpecificStringArray(
